...
export const requestStructuredPairs = async ({ model, chunk, index, total, apiKey, signal }: StructuredPairsParams) => {
 const prompt = [
  ...
  "13. 노래/가사라면 입력 텍스트에 있는 모든 가사 줄을 빠짐없이 항목으로 반환하세요.",
  "14. 노래/가사라면 입력 텍스트에 실제로 있는 반복만 유지하고, 반복되는 줄을 합치거나 삭제하지 마세요.",
  "15. 노래/가사라면 외부 지식, 기억, 알려진 원곡 가사를 이용해 보완하지 마세요.",
  "16. 노래/가사라면 불명확한 부분을 추측해서 복원하지 말고, 입력 텍스트에 적힌 줄과 순서를 보존하세요.",
